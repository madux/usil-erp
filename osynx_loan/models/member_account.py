...
class MemberAccount(models.Model):
    ...
    payment_ids = fields.One2many('loan.payment', 'member_id', string="Payments")
    ...
    monthly_contribution = fields.Monetary(string="Monthly Contribution", currency_field='currency_id')
    number_account = fields.Integer("Number of Account")

    @api.depends('payment_ids')
    def compute_total_capital(self):
        for rec in self:
            rec.total_capital = sum(r.amount for r in self.env['loan.payment'].search([
                ('state', '=', 'validate'),
    ...
    def compute_total_dividend(self):
        for rec in self:
            ...
            if member_count:
                total_payments = sum(r.company_earning for r in self.env['loan.payment'].search([('state','=','validate')]))
    ...
    def action_show_contributions(self):
        return {
            ...
            'context': {
                'default_payment_type_id': self.env.ref('osynx_loan.loan_payment_type_contribution').id,
    ...
    def action_show_payments(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'domain': [('account_id', '=', self.id),
                       # ('payment_type_code', 'in', ['principal','interest'])
                       ],
            'context': {
    ...
    def action_send_contract(self):
        for member in self:
            report = self.env.ref('osynx_loan.action_report_member_contract', False)
            pdf_content, content_type = report.sudo()._render_qweb_pdf(member.id)

            pdf_name = _("Account Contract - %s" % member.name)
            # Sudo to allow payroll managers to create document.document without access to the
            # application
            attachment = self.env['ir.attachment'].sudo().create({
                'name': pdf_name,
            ...
            # Send email to employees
            template = self.env.ref('osynx_loan.mail_template_member_contract', raise_if_not_found=False)
            if template:
                email_values = {
                    'attachment_ids': attachment,
                }
